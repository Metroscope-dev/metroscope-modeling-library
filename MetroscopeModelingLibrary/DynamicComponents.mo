  ...
  */
  ...
  package HeatExchangers
    ...
    //        dW_fg[i]/(dA_fg_tubes + eff_fins*dA_fg_fin) - K_cond_wall*(T_wall_y[N_wall+1,i] - T_wall_y[N_wall,i])/dy_wall = K_cond_wall/a_wall*dy_wall/2*der(T_wall_y[N_wall+1,i]); // check sign
              ...
              - dW_fg[i] - K_cond_wall*(T_wall_y[N_wall+1,i] - T_wall_y[N_wall,i])/dy_wall*pi*D_in*dx*N_tubes = dM_wall*Cp_wall/N_wall/2*der(T_wall_y[N_wall+1,i]); // check sign
              // a[i] = K_cond_wall*(T_wall_y[2,i] - T_wall_y[1,i])/dy_wall*pi*D_in*dx*N_tubes;
              // b[i] =  K_cond_wall*(T_wall_y[N_wall+1,i] - T_wall_y[N_wall,i])/dy_wall*pi*D_out*dx*N_tubes;
              St[1,i] = dM_wall*Cp_wall/N_wall/2*der(T_wall_y[1,i]);
              St[N_wall+1,i] = dM_wall*Cp_wall/N_wall/2*der(T_wall_y[N_wall+1,i]);
            ...
            for j in 2:N_wall loop
              ...
              St[j,i] = dM_wall*Cp_wall/N_wall*der(T_wall_y[j,i]);
            end for;
    ...
    model MonoPhasicHX_1DConduction_quater "Added more info on fins and used ESCOA correlation"
     import MetroscopeModelingLibrary.Utilities.Units;
      import MetroscopeModelingLibrary.Utilities.Units.Inputs;
      import MetroscopeModelingLibrary.Utilities.Constants;
      import CorrelationConstants =
    ...
    equation
      ...
      // ------ Discretization computation loop ------
        for i in 1:N loop
          ...
          // Conduction heat transfer
            - dW_water[i] + K_cond_wall*(T_wall_y[2,i] - T_wall_y[1,i])/dy_wall*pi*D_in*dx*N_tubes = dM_wall*Cp_wall/N_wall/2*der(T_wall_y[1,i]); // check sign
            - dW_fg[i] - K_cond_wall*(T_wall_y[N_wall+1,i] - T_wall_y[N_wall,i])/dy_wall*pi*D_out*dx*N_tubes = dM_wall*Cp_wall/N_wall/2*der(T_wall_y[N_wall+1,i]); // check sign
    ...
    equation
      ...
      annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
            ...
            Text(
              ...
              fontSize=20,
              textString="Flue gas side")}),                         Diagram(coordinateSystem(preserveAspectRatio=false)));
    end MonoPhasicHX_1DConduction_quater;
  end HeatExchangers;
  ...
  package Tests
    ...
    model MonoPhasicHX_1DConduction_quater_test
      import MetroscopeModelingLibrary.Utilities.Units;
      import MetroscopeModelingLibrary.Utilities.Units.Inputs;
      HeatExchangers.MonoPhasicHX_1DConduction_quater
                                         monoPhasicHX_1DConduction_quater(
        N_tubes_row=184,
        Rows=2,
        Tubes_Config=2,
        fg_path_width=14.07,
    ...
    equation
      ...
      connect(monoPhasicHX_1DConduction_quater.water_inlet, cold_source.C_out) annotation (Line(points={{10,9.4},{10,8},{48,8},{48,20},{59,20}}, color={28,108,200}));
      connect(monoPhasicHX_1DConduction_quater.water_outlet, cold_sink.C_in) annotation (Line(points={{-10,9.62},{-10,8},{-48,8},{-48,20},{-59,20}}, color={28,108,200}));
      connect(monoPhasicHX_1DConduction_quater.fg_inlet, hot_source.C_out) annotation (Line(points={{-10,0.6},{-50,0.6},{-50,-40},{-61,-40}}, color={95,95,95}));
      connect(monoPhasicHX_1DConduction_quater.fg_outlet, hot_sink.C_in) annotation (Line(points={{9.8,0.6},{50,0.6},{50,-40},{59,-40}}, color={95,95,95}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(coordinateSystem(preserveAspectRatio=false)),
        experiment(
          StopTime=500,
          __Dymola_NumberOfIntervals=1000,
          __Dymola_Algorithm="Dassl"));
    end MonoPhasicHX_1DConduction_quater_test;
  end Tests;
