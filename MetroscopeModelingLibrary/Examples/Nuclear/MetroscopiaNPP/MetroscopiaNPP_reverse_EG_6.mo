...
model MetroscopiaNPP_reverse_EG_6
  ...
  WaterSteam.Machines.SteamTurbine                           HPT_2(
    ...
    h_in_0=2.73e6,
    h_out_0=2.68e6,
    Q_0=1113) annotation (Placement(transformation(extent={{-507,112},{-489,128}})));
  Sensors_Control.WaterSteam.PressureSensor                   HPT_P_out_sensor(
    Q_0=1113,
    P_0=1940000,
    h_0=2.68e6,
    sensor_function="Calibration",
    causality="LPT1_Cst")                                                      annotation (Placement(transformation(extent={{-466,
            114},{-454,126}})));
  Utilities.Interfaces.RealInput HPT_extract_P1(start=19.4)
        ...
        extent={{-4,-4},{4,4}},
        rotation=270,
        origin={-460,134}),iconTransformation(extent={{-170,22},{-130,62}})));
  Utilities.Interfaces.RealOutput HPT2_Cst annotation (Placement(transformation(
        extent={{-4,-4},{4,4}},
        rotation=270,
        origin={-506,136}), iconTransformation(extent={{-328,-88},{-308,-68}})));
...
equation
  ...
  connect(HP_extract.C_main_out,HPT_2. C_in)
    annotation (Line(points={{-549.4,120},{-507,120}}, color={28,108,200}));
  connect(HPT_2.C_out,HPT_P_out_sensor. C_in)
    annotation (Line(points={{-489,120},{-466,120}}, color={28,108,200}));
  connect(HPT_P_out_sensor.P_sensor,HPT_extract_P1)
    annotation (Line(points={{-460,126},{-460,134}}, color={0,0,127}));
  connect(HPT_2.Cst,HPT2_Cst)  annotation (Line(points={{-506.1,125.6},{-506.1,
          128.8},{-506,128.8},{-506,136}}, color={0,0,127}));
  connect(HPT_2.eta_is, turbines_eta_is) annotation (Line(points={{-504.3,
          126.08},{-504.3,136},{-504,136},{-504,144},{-370,144},{-370,154}},
                                                       color={0,0,127}));
  connect(HPT_2.C_W_out, generator.C_in) annotation (Line(points={{-489,126.72},
          {-480,126.72},{-480,160},{-100.4,160}}, color={244,125,35}));
  connect(HPT_P_out_sensor.C_out, steam_dryer.C_in) annotation (Line(points={{-454,
          120},{-436,120},{-436,119.273}},      color={28,108,200}));
  connect(sink2.C_in, steam_dryer.C_hot_steam)
    annotation (Line(points={{-420,127},{-420,119.273}}, color={28,108,200}));
  connect(steam_generator.feedwater_inlet, Q_feedwater_sensor.C_out)
    annotation (Line(points={{-729,-60},{-704,-60},{-704,-60},{-677,-60}},
        color={28,108,200}));
  connect(steam_dryer.C_hot_liquid, superheater_drains_pipe.C_in) annotation (
      Line(points={{-420,112.727},{-420,60},{-340,60},{-340,-14}}, color={28,
          108,200}));
  connect(superheater_drains_pipe.C_out, deaerator_outlet_pipe.C_out)
    annotation (Line(points={{-340,-34},{-340,-60},{-366,-60}}, color={28,108,
          200}));
...
end MetroscopiaNPP_reverse_EG_6;
