...
model Reheater_NPP_drainsInlet
  ...
  parameter Units.SpecificEnthalpy h_hot_out_0 = 6e5;
    // --- NEW drains initialization parameters ---
  parameter Units.PositiveMassFlowRate Q_drains_0 = 100;
  parameter Units.Pressure P_drains_in_0 = P_hot_out_0;
  parameter Units.Temperature T_drains_in_0 = 273.15 + 120;
  parameter Units.SpecificEnthalpy h_drains_in_0 = 5e5;

  Connectors.Inlet C_drains_in(Q(start=Q_drains_0),P(start=P_drains_in_0   // NEW
))      annotation (Placement(transformation(extent={{-60,70},{-40,90}}),iconTransformation(extent={{135,-75},{155,-55}})));
  ...
  BaseClasses.IsoPFlowModel hot_side_subcooling(
    ...
    T_out_0=T_hot_out_0,
    h_in_0=h_liq_sat_0,
    h_out_0=h_hot_out_0,                        Q_0=Q_hot_0+Q_drains_0,// NEW
    P_0=P_hot_out_0)                                         annotation (Placement(transformation(
        extent={{-23,-23},{23,23}},
        rotation=180,
        origin={-68,20})));
...
//   connect(C_drains_in, hot_side_drains.C_in); // NEW
//
//   // Steam condensate → mixer
//   connect(hot_side_condensing.C_out, mix_hot_to_subcooling.C_in); // NEW
//
//Drains → mixer
//   connect(hot_side_drains.C_out, mix_hot_to_subcooling.C_in); // NEW
//
//Mixer → subcooling inlet
//   connect(mix_hot_to_subcooling.C_out, hot_side_subcooling.C_in); // NEW
  ...
  connect(hot_side_deheating.C_in, C_hot_in) annotation (Line(
      ...
      color={255,0,0},
      thickness=1));
  connect(Kfr_cold, Kfr_cold)
    annotation (Line(points={{-130,50},{-130,50}}, color={0,0,127}));
  connect(Kth_subc, Kth_subc)
    annotation (Line(points={{-150,80},{-150,80}}, color={0,0,127}));
  connect(hot_side_condensing.C_out, hot_side_subcooling.C_in)
    annotation (Line(points={{-3,20},{-45,20}}, color={28,108,200}));
  connect(C_drains_in, hot_side_subcooling.C_in)
    annotation (Line(points={{-50,80},{-26,20},{-45,20}}, color={28,108,200}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-160,-80},
        ...
        coordinateSystem(preserveAspectRatio=false, extent={{-160,-80},{160,80}}), graphics={
          ...
          extent={{-84,10},{-24,-2}},
          textColor={28,108,200},
          textString="Subcooling"),          Text(
          extent={{-150,72},{-90,58}},
          textString="Drains_inlet",
          textColor={28,108,200})}));
end Reheater_NPP_drainsInlet;
