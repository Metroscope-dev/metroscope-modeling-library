...
model Condenser_discretized_test
  ...
  HeatExchangers.Condenser.Condenser_discretized                condenser_discretized(steady_state=true)
...
equation
  ...
  // Parameters
  condenser_discretized.Qv_cold = Qv_cold;

  // Calibrated Parameters
  condenser_discretized.UA = 25e6;

  // Inputs for calibration
  P_cond_sensor.P = P_cond;

  connect(condenser_discretized.C_cold_in, cooling_source.C_out) annotation (Line(points={{10,-0.8},{20,-0.8},{20,-20},{79,-20}}, color={28,108,200}));
  connect(condenser_discretized.C_cold_out, circulating_water_T_out_sensor.C_in) annotation (Line(points={{10,2.8},{16,2.8},{16,0},{22,0}}, color={28,108,200}));
  ...
  connect(condenser_discretized.C_hot_in, P_cond_sensor.C_out) annotation (Line(points={{0,10},{0,20.1},{0,20.1},{0,30}}, color={28,108,200}));
  connect(condensate_sink.C_in, condenser_discretized.C_hot_out) annotation (Line(points={{0,-39},{0,-8}}, color={28,108,200}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            -100},{100,80}})),
                        Diagram(coordinateSystem(preserveAspectRatio=false,
          extent={{-100,-100},{100,100}})));
end Condenser_discretized_test;
