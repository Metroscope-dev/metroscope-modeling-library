...
model Condenser_test
  ...
  HeatExchangers.Condenser.Condenser                            condenser(M_wall=300000)
  ...
  MetroscopeModelingLibrary.Sensors.WaterSteam.PressureSensor P_cond_sensor annotation (Placement(transformation(
        ...
        rotation=270,
        origin={0,40})));
  Modelica.Blocks.Sources.Step step(height=10, startTime=50)   annotation (Placement(transformation(extent={{56,64},{76,84}})));
equation

  // Boundary Conditions
  turbine_outlet.h_out = h_turbine;
  turbine_outlet.Q_out = -Q_turbine + step.y;
