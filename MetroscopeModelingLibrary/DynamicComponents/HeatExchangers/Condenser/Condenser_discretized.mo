...
model Condenser_discretized "Added more info on fins and used ESCOA correlation"
 import MetroscopeModelingLibrary.Utilities.Units;
  ...
  package WaterSteamMedium = MetroscopeModelingLibrary.Utilities.Media.WaterSteamMedium;
    ...
    Units.Temperature T_cold[N +1] "Cooling water outlet temperature";
    Units.Temperature T_cold_avg[N] "Cooling water average temperature";
    Units.SpecificEnthalpy h_cold[N+1];
    Units.Temperature T_hot_in "Steam inlet temperature";
    Units.Temperature T_hot_out "Steam outlet temperature";
    Units.Temperature T_hot_sat "Saturation temperature";
    Units.SpecificEnthalpy h_hot[2, N] "Steam enthalpy";
    Units.Temperature T_wall[N] "Wall temperature";
    // Pressures
    ...
    Units.Power W_cold;
    Units.Power W_hot;
    Units.Power dW_cold[N];
    Units.Power dW_hot[N];
    // Saturation state
...
equation
  ...
  // ------ Boundaries ------
    ...
    hot_side.W = W_hot;
    // Enthalpy
    h_cold[1] = cold_side.h_in;
    h_cold[N+1] = cold_side.h_out;
    hot_side.W = sum(dW_hot);
    for i in 1:N loop
     h_hot[1, i] = hot_side.h_in;
    end for;
  ...
  // Set saturation state
    ...
    for i in 1:N loop
      // Temperature
      T_cold[i+1] = WaterSteamMedium.temperature_ph(P_cold_in, h_cold[i+1]);
      // Average cold water temperature
      T_cold_avg[i] = (T_cold[i] + T_cold[i+1])/2;
      // Heat exchange by node
      dW_cold[i] = UA_cold*(T_wall[i] - T_cold_avg[i]);
      dW_hot[i] = UA_hot*(T_wall[i] - T_hot_sat);
      dW_cold[i] = Q_cold*(h_cold[i+1] - h_cold[i]);
      dW_hot[i] = Q_hot/N*(h_hot[2, i] - h_hot[1, i]);
      // Global energy balance
      if not steady_state then
        dW_cold[i] + dW_hot[i] + M_wall*Cp_wall*der(T_wall[i]) = 0;
      else
        dW_cold[i] + dW_hot[i] = 0;
      end if;
    end for;
