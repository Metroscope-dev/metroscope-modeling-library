...
model Dynamic_SinglePhase_HX
  ...
  parameter Integer N = 6 "Discretization number"
  annotation(Dialog(tab="General", group="Modes"));

  // Design and geometry parameters
  parameter Real r_UA = 39.677 "Ratio of heat conductances between water and fg"
  annotation(Dialog(tab="General", group="Design and geometry"));
  parameter Units.Area A = 3500 "Heat exchange surface"
  annotation(Dialog(tab="General", group="Design and geometry"));
  parameter Units.Mass M_wall = 25379 "Total HX mass"
  annotation(Dialog(tab="General", group="Design and geometry"));
  parameter Units.HeatCapacity Cp_wall = 420 "Tubes specific heat capacity"
  annotation(Dialog(tab="General", group="Design and geometry"));

  // Heat conductances
  Modelica.Units.SI.ThermalConductance UA_fg(start=171696.64) "Fg heat conductance";
  Modelica.Units.SI.ThermalConductance UA_water(start=1476005.2) "Water heat conductance";
  Modelica.Units.SI.ThermalConductance UA(start=155746) "Overall heat conductance";
  ...
  // Initialization
  parameter Units.Temperature T_wall_0 = 517.5455 + 273.15 "Wall temperature" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.Pressure P_water_0 = 70e5 "Water pressure" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.Pressure P_fg_0 = 1e5 "Fg pressure" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.PositiveMassFlowRate Q_water_0 = 85 "Water mass flow rate" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.PositiveMassFlowRate Q_fg_0 = 640 "Fg flow rate" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.Temperature T_water_out_0 = 560 + 273.15 "Water outlet temperature" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.Temperature T_fg_out_0 = 614 + 273.15 "Fg outlet temperature" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.SpecificEnthalpy h_water_out_0 = 3354324.5 "Water outlet enthalpy" annotation(Dialog(tab="General", group="Initialization"));
  parameter Units.SpecificEnthalpy h_fg_out_0 = 912869.94 "Fg outlet enthalpy" annotation(Dialog(tab="General", group="Initialization"));
  ...
  Utilities.Interfaces.GenericReal U annotation (Placement(transformation(
        extent={{-4,-4},{4,4}},
        rotation=180,
...
equation
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, initialScale=0.5),
        ...
        Line(points={{-40,40},{40,40}},  color={0,0,0}),
        Line(points={{-40,20},{40,20}},  color={0,0,0}),
        Line(points={{-40,0},{40,0}},    color={0,0,0})}),       Diagram(coordinateSystem(preserveAspectRatio=false, initialScale=0.5)));
end Dynamic_SinglePhase_HX;
