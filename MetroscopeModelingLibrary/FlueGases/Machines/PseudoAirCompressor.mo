...
model PseudoAirCompressor

  extends MetroscopeModelingLibrary.FlueGases.BaseClasses.FlowModel annotation (IconMap(primitivesVisible=false));
  ...
  // Added
  parameter Integer nb_stages = 5;

  Real gamma_in;
  Real gamma_out;

  Inputs.InputReal gamma;
  Inputs.InputReal deltaT_stage;
  Inputs.InputReal eta_poly;
...
equation
  ...
  // Added
  T_rise = nb_stages * deltaT_stage;
  tau = (1 + T_rise / T_in) ^ ((eta_poly * gamma) / (gamma - 1));
  T_out = T_rise + T_in;
