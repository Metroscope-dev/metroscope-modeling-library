...
model CoolingTower2_reverse
  ...
  input Units.PositiveMassFlowRate waterFlow(start=12.5) "m3/s";
  ...
  input Real airFlow(start=440) "m3/s";

  // Hot Water Outlet

  // Cold Air Outlet
  output Units.Fraction cold_sink_relative_humidity(start=1) "1";
  input Real AirOutletTemp(start=28);
  ...
  //output Real Q_cold;
  //input Real Q_hot(start=12.5);

  // Sensors for Calibration
  input Real WaterOutletTemp(start=32) "deg_C";
