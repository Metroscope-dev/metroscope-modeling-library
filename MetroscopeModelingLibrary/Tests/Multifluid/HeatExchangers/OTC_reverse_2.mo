...
model OTC_reverse_2
  ...
  Utilities.Interfaces.RealOutput Kth annotation (Placement(transformation(
          extent={{-68,76},{-60,84}}), iconTransformation(extent={{-292,68},{-272,
            88}})));
  Utilities.Interfaces.RealExpression
                                  Kfr_cold annotation (Placement(transformation(
          extent={{-64,-42},{-56,-34}}), iconTransformation(extent={{-282,54},{-262,
            74}})));
  ...
  MultiFluid.HeatExchangers.Evaporator evaporator(S_parameter=false)
    annotation (Placement(transformation(extent={{10,-50},{110,140}})));
equation

  hot_source.Xi_out = {0.7481,0.1392,0.0525,0.0601,0.0};
  evaporator.S_eq + superheater.S_eq = 10000;

  connect(Kth, superheater.Kth)
    annotation (Line(points={{-64,80},{-50,80},{-50,40},{-42,40}},
                                                 color={0,0,127}));
  ...
  connect(evaporator.C_cold_out, superheater.C_cold_in) annotation (Line(points
        ={{25,120},{-20,120},{-20,50}}, color={28,108,200}));
  connect(superheater.C_hot_out, evaporator.C_hot_in)
    annotation (Line(points={{0,0},{20,0}}, color={95,95,95}));
  connect(evaporator.C_hot_out, T_hot_out_sensor.C_in)
    annotation (Line(points={{100,0},{172,0}}, color={95,95,95}));
  connect(evaporator.C_cold_in, T_cold_in_sensor.C_out)
    annotation (Line(points={{95,80},{142,80}}, color={28,108,200}));
  connect(evaporator.Kth, Kth) annotation (Line(points={{18,-40},{10,-40},{10,80},
          {-64,80}}, color={0,0,127}));
  connect(Kfr_cold.y, superheater.Kfr_cold)
    annotation (Line(points={{-60,-40},{-42,-40}}, color={0,0,127}));
  connect(Kfr_hot_pipe.Kfr, Kfr_hot.y)
    annotation (Line(points={{-70,4},{-70,18}}, color={0,0,127}));
