...
model GT_louver
  ...
  MetroscopeModelingLibrary.RefMoistAir.BoundaryConditions.Sink
                                                              sink annotation (Placement(transformation(extent={{66,-20},{106,20}})));
  MetroscopeModelingLibrary.RefMoistAir.BoundaryConditions.GT_louvers
                                                                    gT_louvers annotation (Placement(transformation(extent={{-40,-40},{40,40}})));
  ...
  Utilities.Interfaces.RealInput H(start=50) annotation (Placement(transformation(
        ...
        rotation=270,
        origin={16,80}), iconTransformation(extent={{-358,-90},{-318,-50}})));
  Sensors_Control.RefMoistAir.FlowSensor
                                       flowSensor annotation (Placement(transformation(extent={{42,-10},{62,10}})));
...
equation
  ...
  connect(gT_louvers.C_out, flowSensor.C_in) annotation (Line(points={{24,0},{42,0}}, color={0,127,127}));
  connect(sink.C_in, flowSensor.C_out) annotation (Line(points={{76,0},{62,0}}, color={0,127,127}));
end GT_louver;
